...
, CASE WHEN ISNULL(dc.PhonePrimary,'') <> '' THEN PhonePrimary
	WHEN ISNULL(dc.PhoneHome,'') <> '' THEN PhoneHome 
	WHEN ISNULL(dc.PhoneCell,'') <> '' THEN PhoneCell
	ELSE PhonePrimary END  Phone
, CASE WHEN ISNULL(dc.EmailPrimary,'') <> '' THEN EmailPrimary
	WHEN ISNULL(dc.EmailOne,'') <> '' THEN EmailOne 
	WHEN ISNULL(dc.EmailTwo,'') <> '' THEN EmailTwo
	ELSE EmailPrimary END AS Email
...
WHERE [SSB_CRMSYSTEM_Contact_ID] IN (SELECT [SSB_CRMSYSTEM_Contact_ID] FROM [dbo].[vwCRMProcess_DistinctContacts_CriteriaMet])
